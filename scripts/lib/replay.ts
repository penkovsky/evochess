...
export function findMatch(game: EvoChessGame, token: string): Match {
  ...
  // chess.js lists a promoting pawn once per promotion piece, so the same
  // from/to arrives four times. Probe each turn once, or every promotion looks
  // ambiguous against itself.
  const seen = new Set<string>();
  for (const mv of game.legalMoves()) {
    for (const options of optionSets(game, mv.from, mv.to)) {
      const key = `${mv.from}${mv.to}${JSON.stringify(options)}`;
